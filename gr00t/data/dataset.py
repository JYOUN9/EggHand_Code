...
# Source: https://github.com/NVIDIA/Isaac-GR00T/blob/b211007ed6698e6642d2fd7679dabab1d97e9e6c/gr00t/data/dataset.py
# Modified by the EggHand authors: episode task text, temporal windows and dataset statistics.
...
class LeRobotSingleDataset(Dataset):
    ...
    def _get_tasks(self) -> pd.DataFrame:
        ...
        for episode in episodes:
            ...
            instruction = episode["tasks"][0] if "tasks" in episode and episode["tasks"] else ""

            tasks_data.append({"task_index": episode["episode_index"], "task": instruction})
    ...
    def _check_integrity(self):
        ...
        for modality_config in self.modality_configs.values():
            for key in modality_config.modality_keys:
                if key == "lapa_action" or key == "dream_actions" or key == "observation.intrinsics":
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        step_data = self.get_step_data(trajectory_id, base_index)

        if "annotation.task_description" in step_data:
            step_data["text"] = step_data["annotation.task_description"]
            
        return self.transforms(step_data)
    ...
    def get_video(
        ...
        # Get the trajectory index and max length for padding
        trajectory_index = self.get_trajectory_index(trajectory_id)
        max_length = self.trajectory_lengths[trajectory_index]

        # Retrieve timestamps with padding, similar to get_state_or_action
        video_timestamps = self.retrieve_data_and_pad(
            array=timestamp,
            step_indices=step_indices,
            max_length=max_length,
            padding_strategy="first_last",  # Pad with first/last frame's timestamp
        )
        ...
        # Handle cases where the video file has fewer frames than metadata suggests.
        if frames.shape[0] < len(step_indices):
            # Pad the missing frames with the last available frame.
            num_missing = len(step_indices) - frames.shape[0]
            if frames.shape[0] > 0:
                last_frame = frames[-1]
                padding = np.repeat(last_frame[np.newaxis, ...], num_missing, axis=0)
                frames = np.concatenate([frames, padding], axis=0)
            else: # If no frames were returned at all, create an array of zeros.
                raise IOError(f"Could not read any frames for video {video_path.as_posix()} with timestamps.")
    ...
    def get_language(
        ...
        task_indices = [trajectory_id] * len(step_indices)
    ...
    def get_data_by_modality(
        ...
        elif modality == "intrinsics":
            return self._get_intrinsics(trajectory_id, key, base_index)
        else:
            raise ValueError(f"Invalid modality: {modality}")

    def _get_intrinsics(self, trajectory_id: int, key: str, base_index: int) -> np.ndarray:
        step_indices = self.delta_indices[key] + base_index
        trajectory_index = self.get_trajectory_index(trajectory_id)
        max_length = self.trajectory_lengths[trajectory_index]
        
        # In this codebase, key is e.g. "observation.intrinsics"
        if key not in self.curr_traj_data.columns:
             # Just in case, try to look for simple name
             raise ValueError(f"Key {key} not found in parquet columns")
        
        data_array = np.stack(self.curr_traj_data[key])
        return self.retrieve_data_and_pad(
            array=data_array, step_indices=step_indices, max_length=max_length, padding_strategy="first_last"
        )
