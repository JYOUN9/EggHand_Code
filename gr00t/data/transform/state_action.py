...
# Source: https://github.com/NVIDIA/Isaac-GR00T/blob/b211007ed6698e6642d2fd7679dabab1d97e9e6c/gr00t/data/transform/state_action.py
# Modified by the EggHand authors for hand-pose forecasting and public release.
# License: Apache-2.0; see LICENSE and NOTICE.
# RotationTransform: Copyright (c) 2023 Columbia Artificial Intelligence and Robotics Lab; MIT.
# Source: https://github.com/real-stanford/diffusion_policy/blob/548a52bbb105518058e27bf34dcf90bf6f73681a/diffusion_policy/model/common/rotation_transformer.py
# Additional license: licenses/diffusion-policy-MIT.txt.
...
class Normalizer:
    ...
    def forward(self, x: torch.Tensor, k=1.96) -> torch.Tensor:
        assert isinstance(
            x, torch.Tensor
        ), f"Unexpected input type: {type(x)}. Expected type: {torch.Tensor}"
        ...
        if self.mode == "q99":
            ...
            std = k*self.statistics["std"].to(x.dtype)
    ...
    def inverse(self, x: torch.Tensor, k=1.96) -> torch.Tensor:
        assert isinstance(
            x, torch.Tensor
        ), f"Unexpected input type: {type(x)}. Expected type: {torch.Tensor}"
        if self.mode == "q99":
    ...
    def apply(self, data: dict[str, Any]) -> dict[str, Any]:
        for key in self.apply_to:
            ...
            if key in self._normalizers:
                mask = state != 0.0
                normalized = self._normalizers[key].forward(state)
                state = torch.where(mask, normalized, state)
...
class RelativeTrajectoryTransform(InvertibleModalityTransform):
    """
    Transforms absolute action trajectories into trajectories relative to the initial state (t=0).
    The state at t=0 is subtracted from all action steps.
    This helps the model learn the shape of the motion, independent of the starting position.
    """

    # Private attributes to store initial states for unapply
    _initial_states: dict[str, torch.Tensor] = PrivateAttr(default_factory=dict)

    def apply(self, data: dict[str, Any]) -> dict[str, Any]:
        """
        Converts absolute action/state trajectory to relative.
        - action_rel(t) = action_abs(t) - state_abs(t=0)
        - state_rel(t) = state_abs(t) - state_abs(t=0)
        """
        self._initial_states.clear()
        for key in self.apply_to:
            is_action = key.startswith("action.")
            is_state = key.startswith("state.")

            if not (is_action or is_state) or key not in data:
                continue

            # Find the corresponding state key to get the reference coordinate
            state_key = key.replace("action.", "state.") if is_action else key
            if state_key not in data:
                continue

            # The last state in the state_horizon is the state at t=0
            initial_state = data[state_key][-1]
            self._initial_states[key] = initial_state

            # Subtract initial state from the entire trajectory
            data[key] = data[key] - initial_state

        return data

    def unapply(self, data: dict[str, Any]) -> dict[str, Any]:
        """
        Converts relative action/state trajectory back to absolute.
        - action_abs(t) = action_rel_pred(t) + state_abs(t=0)
        - state_abs(t) = state_rel_pred(t) + state_abs(t=0)
        """
        for key in self.apply_to:
            if key in data and key in self._initial_states:
                data[key] = data[key] + self._initial_states[key]
