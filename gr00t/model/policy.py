...
# Source: https://github.com/NVIDIA/Isaac-GR00T/blob/b211007ed6698e6642d2fd7679dabab1d97e9e6c/gr00t/model/policy.py
# Modified by the EggHand authors: forecasting inputs, float32 weights and bfloat16 inference.
...
class Gr00tPolicy(BasePolicy):
    ...
    def apply_transforms(self, obs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for t in self._modality_transform.transforms:
            if hasattr(t, 'mean') or hasattr(t, 'min'):
                print("🧮 Found normalization transform:")
                if hasattr(t, 'min'):
                    print("Min:", t.min)
                if hasattr(t, 'max'):
                    print("Max:", t.max)
    ...
    def _assert_delta_indices(self, delta_indices: np.ndarray):
        ...
        if len(delta_indices) > 1:
            # If all indices are the same (e.g., [0, 0, 0, 0]), the step is 0.
            # This is a valid use case for models that require a sequence but are given a single frame.
            if np.all(delta_indices == delta_indices[0]):
                return
